...
window["getVisualizeLibInst"] = function getVisualizeLibInst(params) {
	function downloadWasm(url, progress) {
		class ProgressStreamSink {
			constructor(progressCallback, total) {
				this.loaded = 0;
			...
			write(chunk) {
				...
				const { loaded: loaded, total: total } = this;
				this.progressCallback({ loaded: loaded, total: total });
			}
			close() {
				const { loaded: loaded, total: total } = this;
				this.progressCallback({ loaded: loaded, total: total });
		...
		function pipeProgress(response) {
			progress && response.clone().body.pipeTo(new WritableStream(new ProgressStreamSink(progress, Number(response.headers.get("Content-Length")))));
			return response;
		}
		return fetch(url).then(pipeProgress);
	}
	params = params || {};
	var Module = typeof Module !== "undefined" ? Module : {};
	Module["postRun"] = [];
	...
	Module["loadWasmError"] = function (error) {};
	Module["instantiateWasm"] = function (imports, successCallback) {
		Promise.resolve()
			.then(function () {
				return downloadWasm(Module["locateFile"](), params.onprogress || function () {});
			})
			.then(function (wasmResponse) {
				return WebAssembly.instantiateStreaming(wasmResponse, imports).then(function (output) {
					successCallback(output.instance);
				});
			})
			.catch(function (e) {
				Module["loadWasmError"](e);
			});
		return {};
	};
	...
	var wasmBinaryFile;
	wasmBinaryFile = "VisualizeJSWasm.wasm";
	...
	var ASM_CONSTS = {
		...
		3264723: () => {
			var _initializeBuffers = function (glCtx, frameBuffer, renderbuffer, frameTexture) {
				...
				glCtx.texImage2D(glCtx.TEXTURE_2D, 0, glCtx.RGBA, frameBuffer.width, frameBuffer.height, 0, glCtx.RGBA, glCtx.UNSIGNED_BYTE, null);
				glCtx.bindRenderbuffer(glCtx.RENDERBUFFER, renderbuffer);
				glCtx.renderbufferStorage(glCtx.RENDERBUFFER, glCtx.DEPTH_COMPONENT16, frameBuffer.width, frameBuffer.height);
				glCtx.framebufferTexture2D(glCtx.FRAMEBUFFER, glCtx.COLOR_ATTACHMENT0, glCtx.TEXTURE_2D, frameTexture, 0);
			...
			Module.isMobileDevice = function () {
				if (/^((?!chrome|android).)*safari/i.test(navigator.userAgent.toLowerCase())) {
					return navigator.maxTouchPoints > 0;
				}
				const regex = /Mobi|Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i;
				return regex.test(navigator.userAgent);
			};
			Module.Viewer.prototype.saveVsfx = function (cb) {
				return this.saveVsfxWrapper(function (offset, length) {
		...
		3267895: $0 => {
			if (!Module["canvas"]) {
				console.error("Canvas is not set to module, please use vsLib.canvas = document.querySelector('#canvas');");
			}
		...
		3268343: () => {
			return Module.isMobileDevice() ? 0 : 3;
		},
		3268387: ($0, $1) => {
			Module.HpTrc.Usd += $1;
	...
	function ___syscall__newselect(nfds, readfds, writefds, exceptfds, timeout) {
		try {
			...
			var allHigh =
				(readfds ? HEAP32[(readfds + 4) >>> 2] : 0) | (writefds ? HEAP32[(writefds + 4) >>> 2] : 0) | (exceptfds ? HEAP32[(exceptfds + 4) >>> 2] : 0);
	...
	var dynCall_fiiij = (Module["dynCall_fiiij"] = function () {
		return (dynCall_fiiij = Module["dynCall_fiiij"] = Module["asm"]["dynCall_fiiij"]).apply(null, arguments);
	});
	...
	var dynCall_viijjiii = (Module["dynCall_viijjiii"] = function () {
		return (dynCall_viijjiii = Module["dynCall_viijjiii"] = Module["asm"]["dynCall_viijjiii"]).apply(null, arguments);
	});
	...
	var dynCall_viiiijiii = (Module["dynCall_viiiijiii"] = function () {
		return (dynCall_viiiijiii = Module["dynCall_viiiijiii"] = Module["asm"]["dynCall_viiiijiii"]).apply(null, arguments);
	});
